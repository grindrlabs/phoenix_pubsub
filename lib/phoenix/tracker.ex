defmodule Phoenix.Tracker do
  ...
  @doc """
  Lists all presences tracked under a given topic, by directly consulting
  the underlying tracker_state's ets table.

  This function may return records from replicas that are temporarily
  down, but is guaranteed not to return records from permdown replicas.
  ...
  def graceful_permdown(tracker_name) do
    for shard_number <- 0..(pool_size(tracker_name)-1) do
      Shard.name_for_number(tracker_name, shard_number)
      |> GenServer.call(:graceful_permdown)
    end
    :ok
  end
  ...
  defp pool_size(tracker_name) do
    [{:pool_size, n}] = :ets.lookup(tracker_name, :pool_size)
    n
  end
